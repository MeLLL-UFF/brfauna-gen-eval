...
def main():
    ...
    tracker = eco2ai.Tracker(
        ...
        experiment_description="Inference with multiple models in Pirá",
        file_name=tracker_file
    )

    # Load dataset
    dataset = load_dataset("paulopirozelli/pira", trust_remote_code=True)
    
    columns_to_remove = [col for col in dataset['train'].column_names if col not in ['question_pt_origin', 'answer_pt_origin']]
    ...
    if 'periquito' in args.model_id:
        ...
        with torch.no_grad():  
            for i in range(len(test_set)):
                original_text = test_set['question_pt_origin'][i]
                ...
                article_data = {
                    ...
                    'reference': test_set['answer_pt_origin'][i],
